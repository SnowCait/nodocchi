...
`SafeIishantenAgainstHighOpenHand` は、High OpenHandThreat 単独の局面で、通常打牌 selector が選んだ打牌後が一向聴で、その打牌そのものが全 `High` target に hard-safe だったため ExpectedSelfTsumoValue の threshold 未満でも押した、という reason です。threshold を満たして押した `ValuableIishantenAgainstHighOpenHand` とは区別されます。Riichi threat と Combined threat の一向聴にはこの reason は出ません。意味は [一向聴の選択打牌 hard-safe 例外](ai/push-pull.md#一向聴の選択打牌-hard-safe-例外) を参照してください。

`SafeTwoShantenAgainstHighOpenHand` は、High OpenHandThreat 単独の局面で、通常打牌 selector が選んだ打牌後がちょうど二向聴で、その打牌そのものが全 `High` target に hard-safe だったため押した、という reason です。根拠は相手の推定打点ではなく選択打牌の hard-safe fact だけです。hard-safe でない二向聴と三向聴以上は `TwoOrMoreShantenAgainstHighOpenHand` のままで、Riichi threat と Combined threat の二向聴にはこの reason は出ません。意味は [二向聴の選択打牌 hard-safe 例外](ai/push-pull.md#二向聴の選択打牌-hard-safe-例外) を参照してください。
...
`iishanten forward metrics` は、通常打牌選択が選んだ打牌が1向聴の場合の前方集計値です。`expected self-tsumo value` は経路確率とテンパイ到達後の期待ツモ支払いを掛けた期待値 [点]、`weighted prospective value` は将来テンパイの確定打点を1手目と最終和了牌の残枚数で重み付けした合計、`weighted tenpai wait` はその枝のテンパイ待ちの残枚数・種類数です。前2つは別の尺度なので、区別して読んでください。production の打牌選択が比較に使った値をそのまま表示し、表示用に2手先探索も打点集計もやり直しません。平均へ正規化した値でも局収支EVでもありません。打点を確定できない枝がある場合は `unknown` で、0点として扱いません。打牌後が1向聴でなければ `none` です。値は configured soft horizon の尺度で、押し引きの threshold とは比較しません。

`push/pull expected self-tsumo value (until ryukyoku)` は、同じ選択打牌を `UNTIL_RYUKYOKU` で評価し直した ExpectedSelfTsumoValue で、1向聴 Push/Fold の threshold と比較する値です。configured horizon を変えても同じ打牌なら同じ値になります。明確な threat が無い局面では評価しないので `not evaluated`、値を確定できなければ `unknown` です。意味は [一向聴の攻撃価値](ai/push-pull.md#一向聴の攻撃価値) を参照してください。
