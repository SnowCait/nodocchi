...
「強いテンパイ」は通常打牌で実際に選んだ牌を切った後の `TenpaiWaitAvailability` から判断します。打牌前14枚の受け入れではなく、見え牌を反映したツモ和了可能な待ちです。要求する残枚数は恒常フリテンと、[攻撃を継続した場合の確定打点](#攻撃継続時の確定打点)で決まります。

| 打牌後テンパイ | 要求する残枚数 |
| --- | --- |
| 恒常フリテン `no` かつ攻撃打点が 5200 点以上 | 3枚以上 |
| 恒常フリテン `no` かつ攻撃打点が 5200 点未満 | 4枚以上 |
| 恒常フリテン `no` で攻撃打点を確定できない | 6枚以上 |
| 恒常フリテン `yes` | 8枚以上 |
| 恒常フリテン unknown | 強いと推測しない |

5200 点は inclusive です。恒常フリテンのテンパイはロンできずツモ依存になるため打点 policy を適用せず、非フリテンより多い残枚数を要求します。
...
攻撃モードはリーチ判断と同じ policy で決め、押し引き側で同じ条件を書き直しません。合法 Reach の有無は legal action を source of truth にし、別経路で Reach 可否を推測し直しません。

| 攻撃モード | 評価する和了状況 |
| --- | --- |
| リーチする手 | ロン・リーチ宣言済み・一発なし・河底なし・裏ドラ0 |
...
打点は生きた待ちごと、さらに和了牌の物理牌 (赤5 / 黒5) ごとに求め、その支払点を残枚数で加重平均します。待ち牌種の間も赤 / 黒 variant の間も同じ残枚数 weight で集約します。残枚数0の variant は平均へ入れません。本場・供託は加えません。名前の付いた役満はその実点数をそのまま使います。
...
生きた variant のどれか1つでも支払点を確定できない場合は、推測で平均を作らず打点を使わない残枚数だけの policy へ落とします。点数計算の入力が不足している、裏ドラが確定しない、ダマでは役が無い、ダマではロンできない、打牌後の手牌を組み立てられない、といった理由はすべてここに含まれ、役なしを0点として平均へ入れることはしません。
