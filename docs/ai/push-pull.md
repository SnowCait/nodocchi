...
生きた variant のどれか1つでも支払点を確定できない場合は、推測で平均を作らず打点を使わない残枚数だけの policy へ落とします。点数計算の入力が不足している、裏ドラが確定しない、ダマでは役が無い、打牌後の手牌を組み立てられない、といった理由はすべてここに含まれ、役なしを0点として平均へ入れることはしません。
