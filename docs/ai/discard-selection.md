...
B の「次打牌」は、その打牌後の1向聴 continuation で選びます。手変わりのツモ後に1向聴を保つ打牌が複数ある場合、受け入れ枚数・種類・形だけの浅い比較では、この枝が実際に集計する「そこからテンパイへ進む価値」を見ていないためです。

pre-acceptance 軸 (Shanten / IsolatedTile / IsolatedHonor) まで同順位の候補それぞれについて
...
到達順の違いで同じ手変わりの評価が変わることはありません。例えば 34567899m 5799p 34s から 5p を切った後、1m と 2m はどちらも1向聴を保つ手変わりですが、浅い比較では 1m を引いた枝だけが引いた牌をそのまま切り返し、その先の一通形へ進む枝を失っていました。

この1向聴 continuation は、2向聴の ExpectedSelfTsumoValue と鳴き判断 (Call / Pass 比較) も共有します。したがって手変わり後の次打牌が変われば、それらの値も同じだけ変わります。2向聴 / 3向聴の枝集合、gate、比較順は変えていません。
...
考え方です。Call / Pass 比較で落ちた候補には押し引きを評価せず、落ちた理由もそのまま残します。

鳴き専用の threat 分類・守備 heuristic・safety 判定・threshold・offense 評価は持ちません。鳴く前後で
他家の情報は増えないので、threat facts は鳴く前の局面から1回だけ作り全候補で共有します。鳴いて
変わるのは自分側の材料だけで、それぞれ鳴き後の打牌選択が既に求めた値をそのまま渡します。

| 鳴き | 押し引きへ渡す選択打牌 | 1向聴の前方集計値 |
| --- | --- | --- |
...
鳴き後の合法打牌で判定します。押し引きのために1向聴の前方評価や terminal scoring をやり直すことは
ありません。
...
明確な threat に対して二向聴以上で降りると通常打牌選択より前に確定できる局面では、production の
