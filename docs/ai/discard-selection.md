...
**現時点では diagnostics 専用で、打牌選択には接続していません。** `CurrentTenpaiOffenseWeightedTotal` の比較にも、リーチ / ダマ判断にも、押し引きにも使いません。継続 bonus も係数も threshold も持たず、`Σ(残枚数 × 継続後の打点)` のような集計値も作りません。現在聴牌の攻撃打点と継続枝はまだ同じ確率模型・同じ horizon に揃っていないため、単純に足したり比べたりできる量ではないからです。
