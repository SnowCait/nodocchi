...
成立役の raw な翻数をそのまま合計し、5翻以上の満貫 / 跳満 / 倍満 / 三倍満や13翻以上の limit へ丸めません。合計が13以上でもその値を保持します。基本点と limit 判定は [基本点と limit](#基本点と-limit) が扱います。ロン / ツモの支払い、責任払い (包)、確定した `HandValue` は [実装済みと未実装](#実装済みと未実装) のとおり未実装です。符は [符](#符) が扱います。
...
親 / 子の倍率、ロン / ツモの支払点、100点単位の切り上げ、本場、供託、責任払い (包)、named 役満の点数化、確定した `HandValue` は未実装です。基本点と limit までで止め、支払点は後続 layer が独立して検証できるようにします。
