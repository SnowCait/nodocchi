...
- ドラの点数化
- decomposition × interpretation ごとの scoring と最も高い解釈の選択
- 本場 / 供託
- 役満の点数化と複合役満の倍率
- 確定した点数
- 確定した `HandValue`
...
基本点だけを入力に取る pure な primitive で、翻数・符・limit を再計算しません。[基本点と limit](#基本点と-limit) が求めた基本点をそのまま渡す composition を想定します。リーチ・一発・場風・残り牌数のような支払いに関係しない `WinningContext` は受け取りません。ロン / ツモは既存の `WinMethod` を再利用し、親かどうかは場風 / 自風から推測せず明示的な事実として受け取ります。

### ルールの一次情報

支払点の semantics も [RiichiLab](https://riichi.dev/docs/local-testing) / [RiichiEnv](https://github.com/smly/RiichiEnv) の4人打ちに合わせます。4人打ち固定で、三麻の支払いは別 scope です。
...
同じ基本点240の子ロンは960 → 1000です。合計を先に求めてから丸めると子ツモも1000になり、実際の支払いとずれます。

| 基本点 | 子ロン | 親ロン | 子ツモ | 親ツモ |
| --: | --: | --: | --- | --- |
| 240 | 1000 | 1500 | 300 / 500 (合計1100) | 500 all (合計1500) |
| 1280 | 5200 | 7700 | 1300 / 2600 (合計5200) | 2600 all (合計7800) |
| 2000 | 8000 | 12000 | 2000 / 4000 (合計8000) | 4000 all (合計12000) |
| 3000 | 12000 | 18000 | 3000 / 6000 (合計12000) | 6000 all (合計18000) |
| 8000 | 32000 | 48000 | 8000 / 16000 (合計32000) | 16000 all (合計48000) |

すでに100の倍数になっている支払いは増やしません。

### 支払い内訳を保持する

単一の合計点へ潰さず、誰がいくら払うのかを保持します。合計は内訳から次のように求めます。
...
ロンの結果にツモの支払いを0として持たせず、ツモの結果にロンの支払いを0として持たせません。後続の診断で0が「該当しない」なのか「0点を払う」なのか曖昧にならないよう、ロン / 親ツモ / 子ツモを別 variant として表します。

支払い内訳は点数の大小を表す型ではないため、field や variant の宣言順を比較へ流用できる derived な順序を持ちません。複数の解釈からどれを採用するかの比較 policy は別責務です。
...
倍率や合計が `u32` で表せない基本点はエラーとして扱い、飽和させた誤った点数を silent に返しません。

### まだ扱わない範囲

本場、供託、責任払い (包) は含みません。合計は他家から直接支払われる base payment だけを表し、本場の +300 / +100 も供託の受け取りも足しません。RiichiEnv は本場まで同じ関数で加算しますが、nodocchi では base payment と分けて後続 layer が加算します。
