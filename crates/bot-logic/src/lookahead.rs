...
//!
//! # 3向聴の Progress 寄与と1向聴 continuation の範囲
//!
//! 3向聴の打牌候補からは
//!
//! ```text
//! 3向聴 → Progress → 2向聴 → Progress → 1向聴 → 1向聴 continuation → 聴牌
//! ```
//!
...
/// 最初のツモで2向聴へ進む枝だけを追う対象の向聴数。
const SANSHANTEN_SHANTEN: i8 = RYANSHANTEN_SHANTEN + 1;
...
/// 探索1段分の仮想ツモ枝。
///
/// 同じ state・horizon・枝の範囲へ別の経路から到達した段は探索済みの枝をそのまま共有するため、
/// 実体ではなく共有参照で持つ。共有するのは探索済みの枝そのもので、枝の内容も集計規則も
/// 持ち方では変わらない。
pub type SharedDraws = Arc<[DrawLookaheadDiagnostic]>;
...
/// 探索内で同一 state の結果を共有する memo。
///
/// 共有するのはどれも「同じ入力なら必ず同じ値になる純関数の結果」だけで、枝も評価値も
/// memo の有無で変わらない。深さごとに別の memo を持たず、どの段の探索も同じ責務のこの1つを
/// 通る。
#[derive(Default)]
struct SearchStateMemo {
    two_shanten: HashMap<ContinuationMemoKey, Option<u64>, CountHasherBuilder>,
    iishanten: HashMap<ContinuationMemoKey, Option<u64>, CountHasherBuilder>,
    next_discard: HashMap<HandState, NextDiscard, CountHasherBuilder>,
    same_shanten_next_discard:
        HashMap<SameShantenNextDiscardKey, Option<SameShantenNextDiscard>, CountHasherBuilder>,
    stats: SearchStateMemoStats,
}

// continuation の値は追う枝の範囲でも変わるため、範囲まで含めて同じ入力だけを共有する。
type ContinuationMemoKey = (CandidateBranch, u32, u32, IishantenContinuationScope);

// 手変わり枝の2手目打牌は state・horizon・その先で追う枝の範囲だけで決まる。
...
pub struct SearchStateMemoStats {
    ...
    pub same_shanten_next_discard_hits: u64,
    pub same_shanten_next_discard_misses: u64,
}

/// 3向聴 Progress 評価が1向聴 state で追う枝の範囲。
///
/// 変わるのはこの1点だけで、ツモ牌の列挙・残枚数・物理牌 variant・打牌比較・terminal
...
impl IishantenContinuationScope {
    // 1向聴 state の continuation が進める枝。この対応だけが2方式の違いになる。
...
/// 3向聴 Progress 評価の探索規模。値には影響せず、要求された場合だけ計上する。
///
/// 数えるのは実際に評価した枝と state だけなので、memo hit で省いた分は含まない。
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct ThreeShantenSearchStats {
    /// 3向聴から2向聴へ進むツモの物理牌 variant 数。
    pub three_to_two_variants: u64,
    /// 2向聴から1向聴へ進むツモの物理牌 variant 数。
    pub two_to_one_variants: u64,
    /// 1向聴 continuation で列挙した Progress 枝の物理牌 variant 数。
    pub iishanten_progress_variants: u64,
    /// 1向聴 continuation で列挙した SameShanten 枝の物理牌 variant 数。
    pub iishanten_same_shanten_variants: u64,
    /// SameShanten 枝の先をもう1段進めた Progress 枝の物理牌 variant 数。
    pub iishanten_downstream_variants: u64,
    /// 仮想ツモ leaf state ([`CandidateBranch::draw_variant`]) を作った回数。
    pub draw_variants: u64,
    /// base 打牌評価の要求回数と、memo に無く実際に評価した回数。
    pub base_evaluation_calls: u64,
    pub base_evaluation_misses: u64,
    /// 向聴 / 受け入れの構造評価を実際に行った回数。
    pub structural_evaluation_misses: u64,
    /// 向聴数を維持するツモ牌の列挙 (shanten calculator) を行った回数。
    pub same_shanten_enumerations: u64,
    /// terminal tenpai のツモ打点評価 (terminal scoring) を行った回数。
    pub terminal_scorings: u64,
}

/// base 打牌評価の入力。seenは全候補の構造上の受け入れ牌種に限って保持する。
/// それ以外のseenはどの候補のremainingにも影響しない。
///
...
impl<'a> LookaheadInputs<'a> {
    pub fn new(
        ...
        Self {
            ...
            root: HandState {
                ...
                // 探索中の state は物理牌 ID の昇順という正規形で持つ ([`HandState`])。起点も
                // ここで揃え、同じ手牌を別の並びで渡した入力が同じ state になるようにする。
                tiles: {
    ...
    /// 1向聴 state の continuation が追う枝の範囲を指定する。
    ///
    /// 既定は [`IishantenContinuationScope::ProgressAndSameShanten`]。3向聴起点の production
    /// 評価は自分で [`IishantenContinuationScope::ProgressOnly`] を選ぶので、この入口を使うのは
    /// 3向聴起点と同じ scope で2向聴 state を直接評価する経路だけ。
    ///
    /// 探索内 memo の key はこの範囲を含むので、指定しても別 scope の結果と混ざらない。
    pub fn with_iishanten_continuation(mut self, scope: IishantenContinuationScope) -> Self {
        self.iishanten_continuation = scope;
        self
    }
    ...
    pub fn with_three_shanten_search_stats(mut self) -> Self {
        self.search_stats = Some(Rc::new(RefCell::new(ThreeShantenSearchStats::default())));
        self
    }

    pub fn three_shanten_search_stats(&self) -> ThreeShantenSearchStats {
        self.search_stats
            .as_ref()
    ...
    // 探索規模の計上。要求されていない経路では何もしない。
    fn count(&self, update: impl FnOnce(&mut ThreeShantenSearchStats)) {
        if let Some(stats) = &self.search_stats {
            update(&mut stats.borrow_mut());
        }
    }

    /// 同一 state / continuation の評価結果を探索内で共有する。
    ///
    /// 共有するのは state・見え牌・河・horizon・追う枝の範囲が同じ入力の結果だけで、枝も
    /// 評価値も指定の有無で変わらない。3向聴起点の production 評価と、深い診断専用の探索が
    /// 使う。既存の2向聴・1向聴 production 評価は指定しないまま。
    ...
    fn base_evaluations(&self, counts: &TileCounts, seen: &CandidateSeen) -> SharedBaseEvaluations {
        self.count(|stats| stats.base_evaluation_calls += 1);
...
pub fn three_shanten_progress_only_self_tsumo_value_for_candidate(
    ...
    three_shanten_value(inputs, evaluation, IishantenContinuationScope::ProgressOnly)
}

/// 次の自摸を待つ3向聴 state から、最初のツモで2向聴へ進む枝だけを 1向聴到達後も Progress
/// だけを追う continuation へ接続した期待支払い [[`crate::self_tsumo::SELF_TSUMO_VALUE_SCALE`]]。
///
/// 評価するのは
///
/// ```text
/// 次の自摸を待つ3向聴 state
/// → 1枚ツモって2向聴へ進む
/// → 既存の2向聴 Progress comparator が選ぶ次打牌 (打牌後も2向聴)
/// → 2向聴 Progress → 1向聴 Progress → テンパイ
/// ```
///
/// だけで、最初のツモで3向聴を維持する枝は追わない。値は打牌候補1件分の
/// [`three_shanten_progress_only_self_tsumo_value_for_candidate`] と同じ
/// [`three_shanten_progress_self_tsumo_value`] を通り、起点が「この打牌候補を切った後の state」
/// ではなく「既に action が終わり次の自摸を待つ state」になるだけ。架空の現在打牌は作らず、
/// `acceptance` はこの state に対して既存 acceptance calculator が返した値をそのまま渡す。
///
/// 3向聴以外の state・ツモ打点や残り自摸機会が欠ける局面・接続先の値を確定できない枝がある
/// 場合は、0 点で補完せず `None`。
pub fn awaiting_draw_three_shanten_progress_only_self_tsumo_value(
    inputs: &LookaheadInputs,
    acceptance: &EffectiveAcceptance,
) -> Option<u64> {
    if acceptance.current_min_shanten() != SANSHANTEN_SHANTEN {
        return None;
    }
    let facts = inputs.self_tsumo_facts()?;
    let branch = CandidateBranch::from_waiting_state(&inputs.root);
...
// 3向聴候補1件の Progress 寄与。2方式は1向聴 continuation の枝の範囲だけが違い、探索も集計も
// この1本を共有する。
fn three_shanten_value(
    inputs: &LookaheadInputs,
    evaluation: &DiscardEvaluation,
    continuation: IishantenContinuationScope,
) -> Option<u64> {
    if evaluation.min_shanten_after_discard() != SANSHANTEN_SHANTEN {
        return None;
    }
    let facts = inputs.self_tsumo_facts()?;
    let branch = CandidateBranch::new(&inputs.root, evaluation)?;
...
// 最初のツモが2向聴へ進む枝だけの寄与。3向聴 state を起点にする経路はどちらもこの1本を通る。
fn three_shanten_progress_self_tsumo_value(
    inputs: &LookaheadInputs,
    ...
    for accepted in &acceptance.tiles {
        ...
        for variant in branch.variants_of(&drawable) {
            ...
            inputs.count(|stats| stats.three_to_two_variants += 1);
            let mut next = inputs.clone();
            next.root = state;
            next.own_future_draws = Some(path.terminal_own_future_draws(facts));
            next.iishanten_continuation = continuation;
...
// 1向聴 continuation が実際に列挙した枝の数。値には影響せず、要求された場合だけ計上する。
fn count_iishanten_continuation_draws(inputs: &LookaheadInputs, draws: &[DrawLookaheadDiagnostic]) {
    if inputs.search_stats.is_none() {
        return;
    }
    let mut progress = 0;
    ...
    for draw in draws {
        match draw.transition {
            DrawTransition::Progress => progress += draw.variants.len() as u64,
            DrawTransition::SameShanten => {
                same_shanten += draw.variants.len() as u64;
                downstream += draw
                    .variants
                    .iter()
                    .filter_map(|variant| variant.downstream.as_ref())
                    .flat_map(|downstream| downstream.draws.iter())
                    .map(|draw| draw.variants.len() as u64)
                    .sum::<u64>();
            }
        }
    }
    inputs.count(|stats| {
        stats.iishanten_progress_variants += progress;
        stats.iishanten_same_shanten_variants += same_shanten;
        stats.iishanten_downstream_variants += downstream;
    });
}

// 3→2の到達先では全合法打牌を列挙し、既存 production の2向聴 metric comparator を使う。
// 比較そのものは合法打牌を明示する入口と共有する。
fn best_two_shanten_progress_discard(inputs: &LookaheadInputs) -> Option<(DiscardEvaluation, u64)> {
    ...
    let evaluations: Vec<_> = base
        ...
        .collect();
    let (index, value) = best_two_shanten_progress_discard_among(inputs, &evaluations)?;
    Some((evaluations[index].clone(), value?))
}

/// 渡した打牌候補の中から、3向聴 production の3→2到達先と同じ2向聴 Progress 比較で最良打牌を
/// 選び、その候補の index と Progress 値を返す。
///
/// 打牌後が2向聴でない候補も比較には並ぶので、選ばれた候補の Progress 値は確定しないことが
/// ある。値を確定できない場合も 0 点で補完せず、index だけを返す。
///
/// 先行軸で敗退が確定した候補だけを外し、Progress で決着すれば後続 metric を作らない。
/// 2向聴 Full gate は呼ばず、Progress の値だけを比較する。unknown cohort の値は確定しない。
/// 1向聴到達後に追う枝は `inputs` の [`IishantenContinuationScope`] のままで、この入口が
/// 決めない。
///
/// 候補集合を自分で列挙できない経路 (鳴き後の合法打牌だけを比較する鳴き判断) が、3向聴
/// production と同じ比較を通るための入口。返す index は渡した `evaluations` に対するもので、
/// 絞り込みで外した候補は選ばれない。
pub fn best_two_shanten_progress_discard_among(
    inputs: &LookaheadInputs,
    evaluations: &[DiscardEvaluation],
...
fn cached_two_shanten_progress(
    ...
    let Some(memo) = &inputs.search_state_memo else {
        return two_shanten_progress_self_tsumo_value_for_candidate(inputs, evaluation);
    };
    let facts = inputs.self_tsumo_facts()?;
    let key = (
        CandidateBranch::new(&inputs.root, evaluation)?,
        facts.unknown_tiles,
...
fn iishanten_continuation_after_progress_draw(
    ...
    let key = if inputs.search_state_memo.is_some() {
        Some((
            CandidateBranch::new(&state, &evaluation)?,
...
impl DrawScope {
    ...
    fn downstream(self) -> DownstreamScope {
        match self {
            Self::Progress => DownstreamScope::None,
            Self::SameShanten { downstream } => downstream,
        }
    }

    // 2手目の打牌後から進める枝。進めない指定では `None`。
    fn downstream_scopes(self) -> Option<&'static [DrawScope]> {
        self.downstream().scopes()
    }
}
...
impl HandState {
    // 手牌と河へ物理牌を1枚足す。並びは常に物理牌 ID の昇順に保つ。
    //
    // 牌の格納順・仮想打牌の順序は手牌評価にも恒常フリテンの集合判定にも影響しないので、同じ
    // 牌の集合へ別の順序で到達した state はこの正規形で一致し、そのまま memo の key になる。
    // 物理牌 ID 自体は落とさないため、赤/黒・見え牌・河の異なる state は混ざらない。
    fn insert_tile(tiles: &mut Vec<TileId>, tile: TileId) {
        let at = tiles.partition_point(|held| held <= &tile);
...
impl CandidateBranch {
    ...
    fn for_discard(state: &HandState, discard: TileType, discards_red_five: bool) -> Option<Self> {
        ...
        Some(Self {
            ...
            discarded: {
                let mut river = state.discarded.clone();
                if let Some(discarded) = discarded {
                    HandState::insert_tile(&mut river, discarded);
                }
                river
            },
        })
    ...
    fn same_shanten_drawables(&self, inputs: &LookaheadInputs) -> Vec<DrawableTile> {
        inputs.count(|stats| stats.same_shanten_enumerations += 1);
    ...
    fn draw_variant(
        ...
        let selected = continuation
            .and_then(|facts| same_shanten_next_discard(inputs, &state, facts, scope.downstream()));
        let (next, downstream) = match selected {
            Some(selected) => (selected.next, selected.downstream),
        ...
        DrawVariantLookaheadDiagnostic {
            ...
            downstream: downstream
                .or_else(|| {
                    downstream_scopes
                        .zip(next.evaluation.as_ref())
                        .and_then(|(scopes, evaluation)| {
                            search(inputs, &state, evaluation, scopes, continuation)
                        })
                        .map(SharedDraws::from)
                })
    ...
    fn state_after_draw(&self, draw: TileType, drawn_tile: TileId) -> Option<HandState> {
        ...
        HandState::insert_tile(&mut tiles, drawn_tile);
...
// 探索では同じ入力へ別の経路から到達した枝で共有する。共有するのは探索済みの枝そのもので、
// 枝の集合も比較も集計も memo の有無で変わらない。
fn same_shanten_next_discard(
    inputs: &LookaheadInputs,
    state: &HandState,
    facts: SelfTsumoFacts,
    downstream: DownstreamScope,
) -> Option<SameShantenNextDiscard> {
    // 先の段を進めない枝はここで打牌を選ばない。memo も引かず、呼び出し側の既存の選択へ戻る。
    if downstream == DownstreamScope::None {
        return None;
    }
    let Some(memo) = &inputs.search_state_memo else {
        return same_shanten_next_discard_uncached(inputs, state, facts, downstream);
    };
    let key = (
        state.clone(),
        facts.unknown_tiles,
    ...
    {
        let mut memo = memo.borrow_mut();
        if let Some(selected) = memo.same_shanten_next_discard.get(&key).cloned() {
            memo.stats.same_shanten_next_discard_hits += 1;
            return selected;
        }
        memo.stats.same_shanten_next_discard_misses += 1;
    }
    let selected = same_shanten_next_discard_uncached(inputs, state, facts, downstream);
    memo.borrow_mut()
        .same_shanten_next_discard
...
fn same_shanten_next_discard_uncached(
    inputs: &LookaheadInputs,
    state: &HandState,
    facts: SelfTsumoFacts,
    downstream: DownstreamScope,
) -> Option<SameShantenNextDiscard> {
    let scopes = downstream.scopes()?;
    let base = inputs.base_evaluations(&state.counts, &state.seen);
    let evaluations: Vec<_> = base
        .iter()
    ...
    let mut searched: Vec<Option<Vec<DrawLookaheadDiagnostic>>> = vec![None; evaluations.len()];
    ...
    Some(SameShantenNextDiscard {
        ...
        downstream: searched[index].take().map(SharedDraws::from),
    })
}

// 手変わり枝の2手目打牌と、その打牌を選ぶときに探索した先の段。同じ枝を2回探索しないため、
// 選択に使った枝をそのまま返す。
#[derive(Clone)]
struct SameShantenNextDiscard {
    next: NextDiscard,
    downstream: Option<SharedDraws>,
}
...
fn next_discard(inputs: &LookaheadInputs, state: &HandState) -> NextDiscard {
    if let Some(memo) = &inputs.search_state_memo {
        let mut memo = memo.borrow_mut();
        if let Some(value) = memo.next_discard.get(state).cloned() {
    ...
    if let Some(memo) = &inputs.search_state_memo {
        memo.borrow_mut()
            .next_discard
            .insert(state.clone(), value.clone());
    }
    value
}
...
fn tenpai_tsumo_value(
    ...
    with_prospective_tenpai(state, evaluation, |tenpai| {
        inputs.count(|stats| stats.terminal_scorings += 1);
...
//
// 将来テンパイ時点の自分の河は「現在の自分の河 + ここまでに切った牌 + この打牌」で、既存
// フリテン基盤へそのまま渡す。フリテン判定はこの module が持たない。河は物理牌 ID の昇順で
// 持ち、恒常フリテンも残枚数も河を集合として見るため並びで値は変わらない。
fn with_prospective_tenpai<T>(
    ...
    let mut discarded_tiles = state.discarded.clone();
    HandState::insert_tile(&mut discarded_tiles, discarded);
...
fn same_shanten_self_tsumo_value(
    ...
    let mut total = 0u64;
    for draw in variant.downstream.as_ref()?.draws.iter() {
...
mod tests {
        ...
        for draw in &candidate.draws {
            for variant in &draw.variants {
                match draw.transition {
                    ...
                    DrawTransition::SameShanten => {
                        for second_draw in variant.downstream.as_ref()?.draws.iter() {
                    ...
                    for second in &second_draw.variants {
                        match second_draw.transition {
                            ...
                            DrawTransition::SameShanten if changes == 2 => {
                                ...
                                for third_draw in deeper.draws.iter() {
    ...
    fn the_hand_change_branch_picks_the_next_discard_by_its_continuation() {
        ...
        for (discard, draw, variant) in variants(&lookahead) {
            ...
            assert_eq!(
                downstream.draws,
                search(&inputs, &state, next, PROGRESS_ONLY, Some(continuation))
                    .unwrap()
                    .into()
            );
    ...
    fn three_shanten_progress_connects_to_existing_two_shanten_progress() {
        ...
        let cached = inputs.clone().with_search_state_memo();
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&cached, evaluation),
            Some(expected)
        );
        let before = cached.search_state_memo_stats();
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&cached, evaluation),
            Some(expected)
        );
        let after = cached.search_state_memo_stats();
        assert!(after.two_shanten_hits > before.two_shanten_hits);
        assert_eq!(after.two_shanten_misses, before.two_shanten_misses);
        let unknown = self_tsumo_inputs(&situation, &UnknownTsumoValuator).with_search_state_memo();
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&unknown, evaluation),
            None
        );
        let before = unknown.search_state_memo_stats();
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&unknown, evaluation),
            None
        );
        assert!(unknown.search_state_memo_stats().two_shanten_hits > before.two_shanten_hits);
        assert_eq!(
    ...
    fn the_progress_only_scope_keeps_the_progress_branches_and_drops_the_same_shanten_branches() {
        // 1向聴 state から追う枝は Progress だけになり、Progress 枝そのものは現行と同じ枝・
        // 同じ集計になる。差は SameShanten 枝の寄与ちょうど分。
        let tiles = iishanten_awaiting_draw_hand();
        ...
        assert_eq!(
            IishantenContinuationScope::ProgressAndSameShanten.scopes(),
            IISHANTEN_CONTINUATION
        );
        assert_eq!(
            IishantenContinuationScope::ProgressOnly.scopes(),
            PROGRESS_ONLY
        );
        ...
        let current_progress: Vec<_> = current
            ...
            .cloned()
            .collect();
        assert_eq!(current_progress, progress_only);
        assert!(
            progress_only
                .iter()
                .all(|draw| draw.transition == DrawTransition::Progress)
        );

        let same_shanten: u64 = current
            .iter()
            .filter(|draw| draw.transition == DrawTransition::SameShanten)
            .flat_map(|draw| draw.variants.iter())
            .map(|variant| same_shanten_self_tsumo_value(variant, facts).expect("確定できる"))
            .sum();
        assert!(same_shanten > 0);

        let full = expected_self_tsumo_value_from_draws(&current, facts).expect("確定できる");
        let progress =
            expected_self_tsumo_value_from_draws(&progress_only, facts).expect("確定できる");
        assert_eq!(full, progress + same_shanten);
        assert_eq!(
            awaiting_draw_expected_self_tsumo_value(&inputs, &acceptance),
            Some(full)
        );
    }

    #[test]
    fn the_progress_only_three_shanten_value_stops_at_the_one_shanten_progress_branches() {
        // 3→2 と 2→1 の枝数は現行と同じで、1向聴の SameShanten 枝と、その先の枝だけが消える。
        let tiles = ids(&[0, 4, 12, 24, 36, 48, 60, 108]);
        let situation = visible_situation(&tiles, fixed(2), Vec::new(), None, None, tiles.clone());
        let evaluation = situation
            .evaluations
        ...
        let current_inputs =
            self_tsumo_inputs(&situation, &FIXED_TSUMO_VALUATOR).with_three_shanten_search_stats();
        let current =
            three_shanten_progress_self_tsumo_value_for_candidate(&current_inputs, evaluation)
                .expect("確定できる");
        let progress_inputs =
            self_tsumo_inputs(&situation, &FIXED_TSUMO_VALUATOR).with_three_shanten_search_stats();
        let progress = three_shanten_progress_only_self_tsumo_value_for_candidate(
            &progress_inputs,
            evaluation,
        )
        .expect("確定できる");

        let full_stats = current_inputs.three_shanten_search_stats();
        let progress_stats = progress_inputs.three_shanten_search_stats();
        assert_eq!(
            full_stats.three_to_two_variants,
            progress_stats.three_to_two_variants
        );
        assert!(full_stats.iishanten_same_shanten_variants > 0);
        assert!(full_stats.iishanten_downstream_variants > 0);
        assert_eq!(progress_stats.iishanten_same_shanten_variants, 0);
        assert_eq!(progress_stats.iishanten_downstream_variants, 0);
        assert!(progress_stats.draw_variants < full_stats.draw_variants);
        assert!(progress_stats.terminal_scorings < full_stats.terminal_scorings);
        ...
        // 同じ memo を共有しても、方式ごとの continuation の値は混ざらない。
        let shared = self_tsumo_inputs(&situation, &FIXED_TSUMO_VALUATOR).with_search_state_memo();
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&shared, evaluation),
            Some(current)
        );
        assert_eq!(
            three_shanten_progress_only_self_tsumo_value_for_candidate(&shared, evaluation),
            Some(progress)
        );
        assert_eq!(
            three_shanten_progress_self_tsumo_value_for_candidate(&shared, evaluation),
    ...
    #[test]
    fn the_progress_only_three_shanten_value_shares_the_unknown_and_shanten_guards() {
        let tiles = ids(&[0, 4, 12, 24, 36, 48, 60, 108]);
        let situation = visible_situation(&tiles, fixed(2), Vec::new(), None, None, tiles.clone());
        let evaluation = situation
            .evaluations
        ...
        // 3向聴以外の候補は対象にしない。
        let two_shanten = two_shanten_candidate_hand();
        let situation = visible_situation(
            &two_shanten,
            fixed(3),
            Vec::new(),
            None,
    ...
    #[test]
    fn the_memo_preserves_physical_tiles_seen_discards_and_horizon() {
        let tiles = two_shanten_candidate_hand();
        let situation = visible_situation(&tiles, fixed(3), Vec::new(), None, None, tiles.clone());
        let inputs = self_tsumo_inputs(&situation, &FIXED_TSUMO_VALUATOR).with_search_state_memo();
        let evaluation = &situation.evaluations[0];
        let value = cached_two_shanten_progress(&inputs, evaluation);
        assert!(value.is_some());

        // 同じ牌を別の並びで渡した入力は入口で同じ正規形になるので、同じ枝を再評価しない。
        let mut reordered_tiles = tiles.clone();
        reordered_tiles.reverse();
        let reordered_situation = visible_situation(
            &reordered_tiles,
            fixed(3),
            Vec::new(),
            None,
        ...
        let mut reordered = self_tsumo_inputs(&reordered_situation, &FIXED_TSUMO_VALUATOR);
        assert_eq!(reordered.root, inputs.root);
        reordered.search_state_memo = inputs.search_state_memo.clone();
        assert_eq!(cached_two_shanten_progress(&reordered, evaluation), value);
        assert_eq!(inputs.search_state_memo_stats().two_shanten_hits, 1);
        ...
        assert_eq!(inputs.search_state_memo_stats().two_shanten_misses, 2);

        // 見え牌・河・赤5の見え方・物理牌が違う state は key として混ざらない。
        let key = &inputs.root;
        let mut other = inputs.root.clone();
        other.seen = other.seen.after_discard(evaluation.discard);
        assert_ne!(*key, other);
        let mut other = inputs.root.clone();
        HandState::insert_tile(&mut other.discarded, tiles[0]);
        assert_ne!(*key, other);
        let mut other = inputs.root.clone();
        ...
        assert!(changed.search_state_memo.is_none());
    ...
    #[test]
    fn three_shanten_discard_candidate_values_match_their_awaiting_draw_helper() {
        // 打牌候補の入口は、その打牌後の state を「次の自摸を待つ3向聴 state」として渡した場合と
        // 同じ枝集合・同じ集計になる。鳴き判断の Call 側と Pass 側が同じ Progress-only の尺度に
        // 揃うのはこの一致による。
        let tiles = ids(&[0, 4, 12, 24, 36, 48, 60, 108]);
        ...
        let evaluation = situation
            ...
            .find(|evaluation| evaluation.discard == TileType::new(27).unwrap())
            .expect("打牌候補がある");
        assert_eq!(evaluation.min_shanten_after_discard(), SANSHANTEN_SHANTEN);
        let progress =
        ...
        let mut visible = concealed.clone();
        visible.push(discarded);
        let awaiting = LookaheadInputs::new(&concealed, fixed(2), &[], None, None)
            .with_visible_tiles(&visible)
            .with_tsumo_valuator(&FIXED_TSUMO_VALUATOR)
            .with_own_future_draws(facts.own_future_draws);

        let acceptance = awaiting_draw_acceptance(&concealed, fixed(2), &visible);
        assert_eq!(acceptance.current_min_shanten(), SANSHANTEN_SHANTEN);
        assert_eq!(
            awaiting_draw_three_shanten_progress_only_self_tsumo_value(&awaiting, &acceptance),
            Some(progress)
        );
    }
    ...
    #[test]
    fn structural_base_memo_matches_uncached_and_separates_seen_and_fixed_melds() {
        let tiles = two_shanten_candidate_hand();
        let inputs = LookaheadInputs::new(&tiles, fixed(3), &[], None, None);
        ...
        assert_eq!(inputs.base_evaluations.borrow().len(), 6);
        ...
        let irrelevant = TileType::all()
            .find(|tile| {
                !structural
                    .tiles
                    .iter()
                    .any(|accepted| accepted.tile == *tile)
            })
            .unwrap();
