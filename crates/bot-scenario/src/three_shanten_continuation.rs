...
//!
//! A (progress+same-shanten) は1向聴到達後も Progress + SameShanten を追う比較用の全枝方式、
//! B (progress-only) は Progress だけを追う production の3向聴軸。値の意味が違うため、
//! どちらの方式の値かを必ず添えて表示する。
...
pub fn format_scenario_comparison(scenario: &Scenario) -> String {
    ...
    let mut lines = vec![
        ...
        "  production discard selection uses B; A is kept for comparison only".to_string(),
        String::new(),
    ];
    lines.extend(format_profile(&progress_and_same_shanten));
    lines.push(String::new());
    lines.extend(format_profile(&progress_only));
    lines.push(String::new());
    lines.extend(format_profile_delta(
        &progress_and_same_shanten,
        &progress_only,
    ));
    lines.push(String::new());
...
fn format_profile_delta(
    ...
    let mut lines = vec![
        ...
        format!(
            "  total elapsed: {} -> {} ({})",
            format_duration(progress_and_same_shanten.total),
            format_duration(progress_only.total),
            format_speedup(progress_and_same_shanten.total, progress_only.total),
        ),
    ];
    for (label, value) in search {
        let a = value(&progress_and_same_shanten.search);
        let b = value(&progress_only.search);
        lines.push(format!("  {label}: {a} -> {b} ({})", format_ratio(a, b)));
    }
    for (label, value) in memo {
        let a = value(&progress_and_same_shanten.memo);
...
fn format_decision(comparison: &ThreeShantenContinuationComparison) -> Vec<String> {
    ...
    for decision in [
        &comparison.progress_and_same_shanten,
        &comparison.progress_only,
    ] {
        lines.push(format!(
...
fn format_capture_comparison(paths: &[String], requests: &[ComparedRequest]) -> String {
    let full: Vec<_> = requests
        .iter()
        .map(|request| request.comparison.progress_and_same_shanten.elapsed)
        .collect();
    let progress_only: Vec<_> = requests
        .iter()
        .map(|request| request.comparison.progress_only.elapsed)
        .collect();
    let full_phase: Vec<_> = requests
        .iter()
        .map(|request| {
            request
                .comparison
                .progress_and_same_shanten
                .three_shanten_elapsed()
        })
        .collect();
    ...
    let mut lines = vec![
        ...
        "  A progress+same-shanten: 1-shanten Progress + SameShanten (comparison only)".to_string(),
        "  B progress-only: 1-shanten Progress only (production)".to_string(),
        "  each scope is measured on its own fresh thread, so neither warms the other".to_string(),
        format!(
    ...
    for request in requests {
        lines.push(format!(
            ...
            request.capture,
            request.request_id,
            format_duration(request.comparison.progress_and_same_shanten.elapsed),
            format_duration(request.comparison.progress_only.elapsed),
            format_speedup(
                request.comparison.progress_and_same_shanten.elapsed,
            ...
            selected_label(&request.comparison.progress_and_same_shanten),
...
fn format_selection_difference(requests: &[ComparedRequest]) -> Vec<String> {
    ...
    {
        lines.push(format!(
            "  {}  request_id={}  progress+same-shanten={} progress-only={}",
            request.capture,
            request.request_id,
            selected_label(&request.comparison.progress_and_same_shanten),
            selected_label(&request.comparison.progress_only),
        ...
        lines.push(format!(
            "    A top: {}",
            format_ranked(&request.comparison.progress_and_same_shanten)
        ));
        lines.push(format!(
            "    B top: {}",
...
fn format_statistics(label: &str, full: &[Duration], progress_only: &[Duration]) -> Vec<String> {
    let a = LatencyStatistics::from_durations(full);
...
fn scope_label(scope: ThreeShantenContinuationScope) -> &'static str {
    match scope {
        ThreeShantenContinuationScope::ProgressAndSameShanten => "A progress+same-shanten",
...
fn format_speedup(full: Duration, progress_only: Duration) -> String {
    if progress_only.is_zero() {
        return "speedup n/a".to_string();
    }
    format!(
        "speedup {:.3}x",
        full.as_secs_f64() / progress_only.as_secs_f64()
    )
}

fn format_ratio(full: u64, progress_only: u64) -> String {
    if full == 0 {
        return "n/a".to_string();
    }
    format!("{:.1}%", progress_only as f64 / full as f64 * 100.0)
}
