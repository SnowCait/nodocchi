...
/// 選択に使った同じ evaluation から構築する。防御 family に対応する診断だけ `Some` になる。
...
///
/// 候補評価まで持つ diagnostic を常に構築するため、現物で決着した場合も exact model の candidate
/// evidence を追加収集する。これは既存 diagnostics 有効時と同じ挙動で、選択結果は変わらない。
